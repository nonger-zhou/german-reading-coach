...
**Phase 2.3 起**：**`/import`** 已可写入 **`articles`**：前端 **`cleaned_text`**（含副标题、**作者署名**（去重后）与正文；**发布时间**仅导入页预览、不入 **`original_text`**）映射到 **`articles.original_text`**；**`raw_pasted_text`** 仅浏览器侧，**不入库**；**无 `published_at` / `subtitle` / `author` 列**（时间、副标题、作者均留在 **`original_text`** 内）。**`/articles/[id]`**、**`/dashboard`** 已读 **`articles`**。**Phase 2.5 起**阅读页手动词/语已写 **`vocabulary_*` / `grammar_*`**；**OpenAI**、**Vercel**、**Chrome 插件 MVP** 仍待办。
